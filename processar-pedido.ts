...
export const handler = async (event: SQSEvent) => {
    console.log('📨 Processando mensagens da fila SQS:', JSON.stringify(event, null, 2));
    ...
    for (const record of event.Records) {
        try {
            ...
            const mensagem = `Pedido ${pedido.id} foi processado e está pronto! Cliente: ${pedido.cliente}, Mesa: ${pedido.mesa}, Total: R$ ${total.toFixed(2)}`;
            ...
            await dynamoClient.send(new UpdateItemCommand({
                ...
                UpdateExpression: 'SET #status = :status, atualizadoEm = :atualizadoEm, comprovanteS3 = :s3Key',
            ...
            console.error(`❌ Erro ao processar mensagem ${record.messageId}:`, error);
